...
from .Available_handle_test import Available_handle
...
from AMA_ANALYSIS.analysis_request_header import add_Cart, into_Sellers_list, uk_use_to_sencond_analysis
...
class UKgetinfo():
    ...
    def get_Price(self):
        ...
        Price_xpath_list = [
            ...
            '//*[@id="price-quantity-container"]/div/div[1]/span[1]/text()',
        ...
        Price_re_list = [
            ...
            '''a-size-base a-color-price a-text-normal">\s*([\s\S]*?)\s*</span>'''
        ]
    ...
    def get_Stock(self):
        ...
        Stock_re_list = [
            ...
            'class="a-size-medium a-color-state">\s*([\s\S]*?)\s*<'
        ]
    ...
    #         '''//*[@id="SalesRank"]/ul/li[1]//text()''',
    #         '''//*[@id="SalesRank"]/td[2]/ul/li[1]//text()''',
    ...
    #         print('Category_result_list',Category_result_list)
    ...
    def get_big_Ranking_Category(self):
        ...
        for re_exp in big_Ranking_Category_re_list:
            ...
            if big_Ranking_Category_reuslt_list:
                try:
                    big_Ranking = int(re.findall('\d+',
                                                 big_Ranking_Category_reuslt_list[0][0].replace(' ', '').replace('\n',
                                                                                                                 '').replace(
                                                     ',', ''))[0])
                except Exception as e:
                    print(e)
    ...
    def get_Link_ASIN(self):
        ...
        Link_Color_ASIN_re_list = [
            'data-defaultAsin="([A-Z0-9]*?)"'
        ]


        Link_Size_ASIN_xpath = [
            '//*[@id="variation_style_name"]/ul//@data-defaultasin',
        ...
        if not Link_Color_ASIN_result_list:
            for re_exp in Link_Color_ASIN_re_list:
                Link_Color_ASIN_result_list = re.findall(re_exp,self.response)
                if Link_Color_ASIN_result_list:
                    Link_Color_ASIN_list = []
                    for Link_Color_ASIN in Link_Color_ASIN_result_list:
                        if Link_Color_ASIN:
...
def create_UK_analysis_obj(all_response, url):
    '''
    ...
    '''
    usa_product_obj = UKgetinfo(all_response, url)
    return usa_product_obj


class UKsecondgetinfo():
    ...
    def Stock(self, ASIN, offerListingID):
        ...
        :return:
        '''
        add_cart_responses = uk_use_to_sencond_analysis.UK_Add_Cart(ASIN=ASIN, offerListingID=offerListingID)
        print('into stock')
        add_OK_re_list = [
            'Added to Basket',
            'Added to Cart'
    ...
    def Available(self, sellers_list_url, ASIN,timeout=20):
        ...
        sellers_list_responses = uk_use_to_sencond_analysis.UK_Into_sellers_list(into_sellers_list_url=sellers_list_url)
...
def UK_second_Analysis(ASIN, stock, sellers_list_url, product_info, offerListingID):
    '''
    没有环境黄金购物车时，进行二次获取信息
    :param ASIN:
    :param Stock:
    ...
    uk_second_product_obj = UKsecondgetinfo(product_info=product_info)

    if 'Usually' in stock or 'ship' in stock:
        print('into Usually')
        uk_second_product_obj.Stock(ASIN=ASIN, offerListingID=offerListingID)

    elif 'Only' in stock:
        print('into only')
        uk_second_product_obj.Only(stock=stock)
        ...
        uk_second_product_obj.Available(sellers_list_url=sellers_list_url, ASIN=ASIN)

    elif 'unavailable' in stock or 'Unavailable' in stock or 'out of' in stock:
        print('into unavailable')
        uk_second_product_obj.Unavaible()

    else:
        uk_second_product_obj.NA()
